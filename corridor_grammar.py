...
from irregular_room_grammar import directional_runs, render_irregular_room as _render_irregular_room
...
def _apply_east_cap_spacing(
    rows: tuple[str, ...],
    cells: frozenset[Point] | None = None,
) -> tuple[str, ...]:
    """Reserve one blank screen column before north/background east caps only."""
    updated = list(rows)
    if cells is None:
        for y, row in enumerate(updated[:-1]):
            updated[y] = row.replace(EAST_CAP_UNDERSIDE_SOURCE, EAST_CAP_UNDERSIDE_TARGET)
        return tuple(updated)
    for boundary_y, _start_x, _end_x in directional_runs(cells, "north"):
        underface_y = boundary_y * SECTION_STRIDE_Y + 1
        if 0 <= underface_y < len(updated):
            updated[underface_y] = updated[underface_y].replace(
                EAST_CAP_UNDERSIDE_SOURCE,
                EAST_CAP_UNDERSIDE_TARGET,
            )
    return tuple(updated)
...
def render_irregular_room(cells: frozenset[Point]) -> tuple[str, ...]:
    ...
    rows = _apply_east_cap_spacing(_render_irregular_room(cells), cells)
