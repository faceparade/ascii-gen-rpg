...
def test_internal_width_consistency() -> None:
    ...
    # middle v2: the box is rectangular — every row (including the north top
    # row) is full width (34) because v2 closes the box with a '|' right wall.
    shell = RoomShell(variant="middle")
    rows = shell.render()
    for r in range(len(rows)):
        assert_equal(len(rows[r]), shell.full_width, f"middle v2 row {r} should be full width (34)")
    assert_equal(len(rows[0]), shell.full_width, "middle v2 north top row is full width (box closed)")
...
def test_north_opening_delegates_to_northwall() -> None:
    ...
    # The v2 top row closes the box differently from the bare NorthWall top,
    ...
    assert "j" in shell_rows[0] and ",t" in shell_rows[0], \
        "v2 top row should carry the NorthWall opening carve (j / ,t)"
    assert wall_rows[0] in (shell_rows[0] + "|") or shell_rows[0].startswith(wall_rows[0][:-1]), \
        "v2 top row should embed the NorthWall opening carve"
    assert_equal(shell_rows[1], "|" + wall_rows[1][1:], "v2 underside row keeps NorthWall carve with closed left wall")
    print("PASS north opening carve embedded in v2 top row")


def test_west_east_openings_carve_side_walls() -> None:
    # West opening: a doorway through the left wall band, rows 4..6.
    # v2 west wall is solid '|' on every body row.
    ...
    # Rows outside the opening keep the solid v2 west wall char '|'.
    assert_equal(west[3][0], "|", "west wall col0 above opening start unchanged")
    assert_equal(west[7][0], "|", "west wall col0 below opening unchanged")
    ...
    # Rows outside the opening keep the solid east wall char '|'.
    assert_equal(east[3][-1], "|", "east wall col right above opening start unchanged")
    assert_equal(east[8][-1], "|", "east wall col right below opening unchanged")
...
def test_platformless_fallback_removed() -> None:
    # v3 has no embedded platform; the 'has_platform' field was removed.
    shell = RoomShell(variant="middle")
    ...
    print("PASS v3 middle shell has no platform field (11 rows)")
...
if __name__ == "__main__":
    ...
    test_platformless_fallback_removed()
